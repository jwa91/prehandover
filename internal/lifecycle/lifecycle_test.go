...
func TestDecisionAdaptersRenderFailure(t *testing.T) {
	for _, harness := range []string{"claude", "codex"} {
		t.Run(harness, func(t *testing.T) {
			...
			if err := adapter.Encode(MomentHandover, OutcomeFromRun(failingRun()), &out); err != nil {
				t.Fatal(err)
			}
...
func TestAdaptersRenderPassAsEmptyOutput(t *testing.T) {
	...
	for _, harness := range []string{"claude", "codex", "cursor"} {
		t.Run(harness, func(t *testing.T) {
			...
			var out bytes.Buffer
			if err := adapter.Encode(MomentHandover, OutcomeFromRun(pass), &out); err != nil {
				t.Fatal(err)
			}
			if out.Len() != 0 {
...
func TestDecisionAdapterDecode(t *testing.T) {
	...
	inv, err := adapter.Decode(MomentHandover, []byte(`{
		"cwd": "/repo",
		"session_id": "s1",
		"turn_id": "t1",
...
func TestDecodeEmptyInputFallsBackToCurrentWorkingDirectory(t *testing.T) {
	...
	for _, harness := range []string{"claude", "codex", "cursor"} {
		t.Run(harness, func(t *testing.T) {
			adapter, _ := ForHarness(harness)
			inv, err := adapter.Decode(MomentHandover, nil)
			if err != nil {
				t.Fatal(err)
			}
